  ...
  write(ini,'(i1, a2, a37,  a48)') iP, ' ', chB(1:37),  &
...
!  write(ini, *)'G0.rsol           ** IC file read when type of IC = 0A'  
  ...
  write(ini,'(a6, 2es12.4, f6.2, a64)') chA, tol_max,tol_min, tol_Lq,  &
  ...
  elseif(max_iter == -3) then
     ...
     write(ini, '(a6,a8,es12.4,a15,a57)' ) 'newton  ', chA(1:6), tol ,'   50    50   ', &
          '** non_solver, stop_crit, tol, max_iter, min C update'
     
     read(dat,*) chA
     if(chA /= "lin") then
        if(time_disc == 'STDG') then
           write(ini, *) 'none      GMRES_ILU   0.1                   ', &
                '** linear MultiGrid?,  lin_solver   tol ' 
        else
...
!EOF
  ...
  if(ch_model == 'NSe' .and. command_argument_count() == 33) then
     ...
     write(ini,'(2i3,4es14.6,a19, f5.2,a8, f5.2)') niBC, 0, 1., v1,  v2,  p, &
          ' ** Inlet,  M = ', rmach, ', alpha =', alpha
     if(nbc > 1) &
          write(ini,'(2i3,4es14.6,a19, f5.2,a8, f5.2)') niBC+1, 1, 1., v1,  v2,  p, &
