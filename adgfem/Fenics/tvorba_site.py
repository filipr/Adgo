...
gridName ='square01-square01-03.grid'
subGridName = 'square01.subgrid'
...
#    def inside(self, x, on_boundary):
#        return (near(x[0], p3[0]) and ( near(x[1],p3[1]) or near(x[1],p4[1])) ) or \
#               (near(x[0], p4[0]) and ( near(x[1],p3[1]) or near(x[1],p4[1])) )
...
#geometry = mshr.Circle(center, radius, 4) #+ mshr.Rectangle(Point(0.0, 0.0), Point(L, W))
p1= Point(0.0, 0.0)
p2= Point(1.0, 1.0)
p3 = Point(0.1, 0.1)
p4 = Point(0.3, 0.3)
...
#domain.set_subdomain(1, mshr.Circle(center, radius, 4) )
...
#vertexMark = VertexFunction("bool", mesh2d)
#vertexMark.set_all(False) 
#rectangle.mark(vertexMark, True)

#innerVertex = []
#for v in vertices(mesh2d): 
#   if (vertexMark[ v.index() ] == True):
#      innerVertex.append( v.index() )
#      print 'v.index' , v.index(), coords[ v.index()  ]
#      
