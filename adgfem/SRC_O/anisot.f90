...
contains
  ...
  subroutine AdaptMesh_Angener(metric, identical_AMA_grids)
    ...
    !if(AMA%adapt_level == 1) AMA%ifig = 0   !     ...     vizualization
    ...
    if(len(lines_file) > 3) then
       ...
       if( AMA%nconstr > 0) then
          read(icon, *) not_used
          allocate(AMA%constr(1:AMA%nconstr, 1:2, 1:2))
          allocate(AMA%iconstr(1:AMA%nconstr))
  ...
  subroutine ReadingOfParamets(metric )
    ...
    if(metric) then
       ...
       if(state%modelName == 'scalar' .or.state%modelName == '2eqs' .or.state%modelName == 'porous') then
          ...
          ! anis_hp for NSe
          AMA%pos = 0.001       ! criterion 2|K| /sum l^2 > pos
          AMA%posW = 0.001
