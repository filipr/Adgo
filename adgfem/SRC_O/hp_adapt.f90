...
contains
  ...
  subroutine AdaptMesh_hp_HG( )
    ...
    !print*,' DeletehpGrid'
    !call PlotMesh_hp(hp_grid, 1)  !!state%space%adapt%adapt_level)
  ...
  subroutine DRefineGrid(hp_grid)
    ...
    do i=1,hp_grid%nelem
       ...
       if(elem%hsplit == -4 .and. elem%RGhistory > 0 ) then
          !!write(*,'(a10,2i4, 6es12.4)') 'elem4r4r4:', elem%i,  elem%RGhistory
...
!!!print*,'....',k,elem%flen, rem(:)
  ...
  subroutine RedHGderefinement(hp_grid, elem)
    ...
    elem%HGnode = .true.
    !if(allocated(elem%HGface)) deallocate( elem%HGface)
    allocate(elem%HGface(hg_face:hg_index,1:flen) )
    elem%HGface(:,:) = 0

    !if(allocated(elem%HGvertex) ) deallocate(elem%HGvertex)
    allocate(elem%HGvertex(1:elem%type+1) )
...
!!!FindMiddleHGNode = i+1
  ...
  subroutine DeletehpGrid(hp_grid)
    ...
    do i=1,hp_grid%npoin
       deallocate ( hp_grid%node(i)%x)
       !deallocate ( hp_grid%node(i)%xcur )
    enddo
  ...
  subroutine PlotMesh_hp(hp_grid, isol1)
    type(hp_mesh), intent(in) :: hp_grid
    integer, intent (in)  :: isol1
    ...
    character(len=5) :: ch5
    real :: xc(1:nbDim)
    integer :: i,j, flen, lnd, chlen , ip, isol
