...
contains
  ...
  subroutine gmres(nsize, x, d, nit,tol, prod, precond, nvec,  &
    ...
    real, dimension(:), allocatable ::  vk
    !ccc
    !ccc   Small vectors
    !ccc
    real amat(mdim,mdim),bvec(mdim),vv(mdim)
    real hmat(mdim,mdim),hvec(mdim)
    interface
    ...
    real dnorm, qtq, ss, sum0,rr, bi, hi

    m = nsize

    allocate( qd(1:nsize,1:mdim), vk(nsize) )
...
1000 format('Pass...',i6,' residual**2 = ',1pd10.3)
    if(it.eq.0) then
       ...
       if(iout.eq.1) then
          ...
          print*,'#####################################'
          write(*,'(i5,6es12.4)')  &
               it,sum,sum0,tol,tol2,tol**2*sum0,tol2**2
       endif
    endif
    ...
    !write(*,'(a6,6es12.4)') 'gm???1',sum, tol**2*sum0, sum,tol2**2,sum0
    if( (sum.le.tol**2*sum0 .and. sum.le.tol2**2) .or. sum/nsize .le. 1.E-50 ) then
    ...
    do i=1,nvec
       ...
       if(sum.le.tol**2*sum0 .and. sum.le.tol2**2) then
          !print*,'gm: ESCAPE ??'
          iflag=1
          kvec=i
          goto 5000
...
9000 continue
    if(state%space%adapt%adapt_method /= 'ALG2') &
         print *,'Gmres  failed to converge in ',it,' iterations'
       ! evaluation of the error using backward analysis
       normX = VectorNorm(x(:) )
