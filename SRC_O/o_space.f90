module space_mod
   ...
   subroutine initDGdata( this, disc_space, sigma, deg )
   ...
   subroutine createQuadRules( this )
      ...
      !    i = this%Qdeg(j, 1)
         ...
         if(.not. this%V_rule(i)%def)  call this%V_rule(i)%createVrule(i)
...
!     stop
    ...
    do i=0,  maxLrule
       ...
       ! Create_L_rule(state%space%L_rule(i), i)
...
!    write(*,'(a4,10e10.3)') '??>.', this%G_rule(2)%phi(3,2,1:10,2)
    ...
    !   allocate(void(1:dof,1:nbDim,1:Qdof) )
       ...
       call PHIorthonormalNew(Qdof, p0, len, dof, this%GScoeff(len, 1:dof, 1:dof), &
        this%L_rule(iq)%phi(1:dof, 1:Qdof) )
