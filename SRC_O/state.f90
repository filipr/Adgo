...
module define_state
  ...
  !  !replaced by NonlinearSol_t
  !  !> structure with parameters of the Newton method
  !  type :: Newton_type
  ...
  !  end type Newton_type

  !> main data structure
  type, public :: solver_state

     logical :: SP   ! saddle point problem

     !character(len=20) :: Set_R_s

     type(rtn_fe), dimension(:), pointer :: RTN ! RTN elements
     type(basis_rtn_fe), dimension(:), pointer :: loc_RTN ! local RTN elements
     real, dimension(:), pointer :: A     ! matrix for conforming FEM
     ! character :: time_method             ! 'E'= explicit, 'I' = implicit,'S'=semi-implicit
     ...
     real, dimension(:), allocatable :: etaRn_sum  ! \sum_{j=1}^n (etaRj)^2,   n = current time lvl
     real, dimension(:), allocatable :: etaDFn_sum  ! \sum_{j=1}^n (etaDFj)^2
     real, dimension(:), allocatable :: etaNC1_sum  ! \sum_{j=1}^n (etaNC1j)^2
     real, dimension(:), allocatable :: etaNC2_sum  ! \sum_{j=1}^n (etaNC2j)^2
     real, dimension(:), allocatable :: etaOsc_sum  ! \sum_{j=1}^n (etaOscj)^2
     real, dimension(:), allocatable :: eta    ! global error estimator

     real, dimension(:,:), allocatable :: estim   ! total error estimator
     ...
     real, dimension(:), allocatable :: L_estim   ! estim of the error for 1 time level
...
!     real :: G_jumps, G_jumps1            ! evaluation of the jumps !UNUSED FR
     ...
     real :: CPU_estim2                      ! CPU time necessary for particular opeartions
     real :: CPU_constaint

     integer :: no_refused_steps              ! number of refused time steps

     !type(Newton_type), pointer :: Newton     ! parameters of the Newton methods
     ...
     logical :: dual ! to distinguish if the primal or dual problem is computed
...
subroutine printSpaceErrorEstims( this )
  ...
  !TODO FERROR DWR method init should be thought through in details !!!
  if(this%space%estim_space == 'DWR') print*, &
       'initialization of DWR should be implemented in initSpaceErrorEstim ??? '
  !    if(this%estim_space == 'DWR') then
  !
  !    allocate( this%DWR )
  !      !dual_deg - primal deg
  !      this%DWR%plus = 1
  !      !setting the tarFunc
  !      this%DWR%lin_functional = .false.
  !      this%DWR%id = floor(this%Lq)
  !      select case(this%DWR%id)
  !         case(1)
  ...
  !          !  this%DWR%xy_coord(1:2) = (0.5, 0.5)
  !            this%DWR%lin_functional = .true.
  !         case(4:)
...
subroutine initErrorEstim( this )
  ...
  write(debug,*) 'initErrorEstims called!'
  print*, 'initErrorEstims + initEstim are now done -> should be joined'
