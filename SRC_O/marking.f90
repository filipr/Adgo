...
contains
  ...
  subroutine MarkElements( )
    ...
    do i = 1, grid%nelem
       ...
       if (state%space%estim_space /= 'pNeu') then
          ...
          if( state%time%disc_time /= 'STDG') then
             elem%estimST = (elem%estim_loc + elem%jumpsJh)**0.5
          else
             elem%estimST = elem%estim_loc**0.5
          endif
       ...
       elseif (state%space%estim_space == 'pNeu') then
          ...
          ! 2nd variant
          !if(elem%deg >= 2) elem%reg = elem%eta(P_pot,1) / elem%eta(P_potP,1)

          !!write(state%space%adapt%adapt_level+650,*) elem%xc(:), elem%estimST, elem%reg
          ...
          !    if( k > 0) then
          !       elem1 => grid%elem(k)
          !       !!elem%estimST = elem%estimST +  elem1%eta(P_tot, 1) * weig
          !        if(elem%deg > 1) &
          !             elem%reg =  elem%reg + weig  &
          !             * elem1%eta(P_F_p1, 1)/elem1%eta(P_F_p2,1)/ elem1%diam**0.5
          !       ipoc = ipoc + 1
          !    endif
          ! enddo
          ! !!elem%estimST = elem%estimST / (1. + ipoc *weig)
          ! if(elem%deg > 1) elem%reg = elem%reg / (1. + ipoc *weig)

          !!write(state%space%adapt%adapt_level+750,*) elem%xc(:), elem%estimST, elem%reg

          ! seting tolerances for h-p decision

          ! 1st variant
          !elem%regT0 = 1.25 * elem%diam**1.00                ! elem%reg <  elem%regT0  ==> p-refinement
          ...
          ! 2nd variant
          !elem%regT0 = 0.25             ! elem%reg <  elem%regT0  ==> p-refinement
          !elem%regT1 = 1.   ! elem%reg <  elem%regT1  ==> h->p substitution
          !elem%regT2 = 2.    ! elem%reg >  elem%regT2  ==> p-DErefinement
          ...
          ! !elem%regT0 = elem%diam**(-2)    ! elem%reg <  elem%regT0  ==> p-refinement
          ! elem%regT0 = elem%diam**(+1)    ! elem%reg <  elem%regT0  ==> p-refinement
          ! elem%regT1 = elem%diam**( 0)    ! elem%reg <  elem%regT1  ==> h->p substitution
          ! elem%regT2 = elem%diam**(-1)    ! elem%reg >  elem%regT2  ==> p-DErefinement


       !else
       !   print*,'Unknown variant of marking in marking.f90'
       !   stop
       endif

       rez_estim = rez_estim + elem%estimST * elem%estimST


       if(ityp == 3) scale = (elem%area/state%space%domain_volume)**0.5
       !write(100+state%space%adapt%adapt_level,*) elem%xc(:),elem%estimST,tol_max *scale, &
       ...
       !!if( state%type_IC == 8 )  elem%estimST = elem%estim_loc


    enddo

    if(ityp == 4) tol_max = maxval(grid%elem(:)%estimST)
    ...
    do i = 1, grid%nelem
       ...
       if(elem%estimST > tol_max *scale  ) then

          if(elem%reg <= elem%regT0 .and. elem%deg < MaxDegreeImplemented-1 ) then
             ! solution is regular => p-refinement
             elem%psplit = 1

             !write(200+state%space%adapt%adapt_level,*) elem%xc(:),elem%estimST,elem%estim_loc,elem%reg
             ...
             !write(300+state%space%adapt%adapt_level,*) elem%xc(:),elem%estimST,elem%estim_loc,elem%reg

          endif

       elseif(elem%estimST < tol_min*scale ) then
          ! NOT YET CHECKED
          if( elem%deg > MinDegreeImplemented)  elem%psplit = -1

          !write(400+state%space%adapt%adapt_level,*) elem%xc(:),elem%estimST,elem%estim_loc,elem%reg, elem%regT2
          !write(*,'(8es12.4)') elem%xc(:),elem%estimST,elem%estim_loc,elem%reg, elem%regT2
       endif
