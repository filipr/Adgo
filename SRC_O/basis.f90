...
!
!    Qdeg = T_rule%Qdeg
!    !G_rule => state%space%G_rule(Qdeg)
!  !  Gdof = T_rule%Qdof
!
!    dof = MaxTimeDegree + 2 ! 2nd +1 for Tdof_plus
!
!    ! time basis functions
!    allocate(T_rule%phi(1:dof, -2:Qdeg) ) ! at Gauss/Radau integ nodes, -1,0  =  phi at end points, -2 special point if needed
!    allocate(T_rule%Dphi(1:dof, -2:Qdeg) )
!    !!!!allocate(T_rule%phiB(1:dof,1:2) )   ! at end points
!
!    T_rule%phi(1:dof, -2) = 0.
!    T_rule%Dphi(1:dof, -2) = 0.
!
!    do j=-1,Qdeg
!       if(j == -1) then
!          t = 0.
...
!       endif
!       !do i=1,dof
!          call EvalTrulePhi(t, dof, T_rule%phi(1:dof,j), T_rule%Dphi(1:dof,j) )
!       !end do
!
!    enddo
!
...
!    ! print*,'###########',dof,Gdof
!    ! write(*,'(a6,i5,30es13.5)') 'nodes:',0,T_rule%lambda(-1:Gdof)
!    ! do i=1,dof
!    !    write(*,'(a6,i5,30es13.5)') 'phi_',i,T_rule%phi(i,-1:)
!    ! enddo
!    ! print*,'.....................'
!    ! ! test of the L^2-orthogonality
!    ! do i=1,dof
!    !    do j=1,dof
!    !       t = dot_product( T_rule%weights(1:Gdof), T_rule%phi(i,1:Gdof)*T_rule%phi(j,1:Gdof) )
...
!
!    !do i=1,Gdof
!    !   write(10+Qdeg,'(30es13.5)') G_rule%lambda(i), T_rule%phi(1:dof, i)
!    !enddo
!
!  end subroutine SetTrulePhi
!
!  subroutine Write_TrulePhi()
