...
!   steep = 2./state%space%h
  ...
  subroutine Set_f_s_Euler(ndimL, nbDim, Qdof, w, f_s, xi)
  ...
  ! !> compute the Euler fluxes for wet steam case \f$ f_s({\bf w}),\quad s=1,2\f$
   subroutine Set_f_s_WS(ndimL, nbDim,Qdof, w, f_s, xi)
     ...
     f_s(1:Qdof, :, :) = 0.
     call Set_f_s_Euler(4, nbDim, Qdof, w, f_s(1:Qdof, 1:2, 1:4), xi)
  ...
  !> for Euler fluxes
  subroutine Set_A_s_Euler(ndimL, nbDim, Qdof, w, A_s, xi)
  ...
  !> compute matrices \f$ A_s = \frac{D{\bf f_s}({\bf w})}{D{\bf w}},\quad s=1,2\f$
  !> for wet steam
  subroutine Set_A_s_WS(ndimL, nbDim, Qdof, w, A_s, xi)
    integer, intent(in) :: Qdof, nbDim, ndimL
    real, dimension(1:Qdof, 1:ndimL), intent(in):: w !state  w in #Qdof nodes
    real, dimension(1:Qdof,1:nbDim,1:ndimL,1:ndimL), intent(inout) :: A_s
                                               ! matrices A_s in  -- " --
   real, dimension(1:Qdof,1 :nbDim), intent(in) :: xi
    ...
    call Set_A_s_Euler(4, nbDim, Qdof, w, A_s(1:Qdof,1:nbDim,1:4,1:4), xi)
  ...
  !> in integ nodes
  subroutine Set_K_sk_NS(ndimL, nbDim, Qdof, w, Dw, Re_1, K_sk)
    integer, intent(in) :: ndimL, nbDim, Qdof
    real, dimension(1:Qdof, 1:ndimL), intent(in):: w !state  w in #Qdof nodes
  ...
  !> in integ nodes
  subroutine Set_K_sk_WS(ndimL, nbDim, Qdof, w, Dw, Re_1, K_sk)
    integer, intent(in) :: ndimL, nbDim, Qdof
    real, dimension(1:Qdof, 1:ndimL), intent(in):: w !state  w in #Qdof nodes
    real, dimension(1:Qdof, 1:ndimL, 1:nbDim), intent(in):: Dw !state  Dw in #Qdof nodes, not USED
    real, dimension(1:Qdof), intent(in) :: Re_1        ! inverse of Reynolds number
    ...
    call Set_K_sk_NS(4, nbDim, Qdof, w(1:Qdof, 1:4), Dw(1:Qdof,1:4,1:nbDim), Re_1, K_sk(1:Qdof,1:nbDim,1:nbDim,1:4,1:4))
  ...
  !> in integ nodes
  subroutine Set_R_s_NS(ndimL, nbDim, Qdof, w, Dw, Re_1, R_s)
    integer, intent(in) :: Qdof, nbDim, ndimL
    real, dimension(1:Qdof, 1:ndimL), intent(in):: w !state  w in #Qdof nodes
  ...
  !> in integ nodes
  subroutine Set_R_s_WS(ndimL, nbDim, Qdof, w, Dw, Re_1, R_s)
    integer, intent(in) :: Qdof, nbDim, ndimL
    real, dimension(1:Qdof, 1:ndimL), intent(in):: w !state  w in #Qdof nodes
    real, dimension(1:Qdof, 1:ndimL, 1:nbDim), intent(in):: Dw !state  Dw in #Qdof nodes
    real, dimension(1:Qdof), intent(in) :: Re_1        ! inverse of Reynolds number
    ...
    call Set_R_s_NS(4, nbDim, Qdof, w(1:Qdof,1:4), Dw(1:Qdof,1:4,1:nbDim), Re_1, R_s(1:Qdof,1:nbDim,1:4))
