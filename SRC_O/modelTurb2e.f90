...
contains
  ...
  subroutine Set_f_s_Turb2e(ndimL, nbDim, Qdof, w, f_s, xi)
    ...
    if(nbDim == 2) then
       call Set_f_s_Euler(nbDim+2, nbDim, Qdof, w(1:Qdof,1:nbDim+2), &
            f_s(1:Qdof, 1:nbDim, 1:nbDim+2), xi )
    elseif(nbDim == 3) then
...
!            f_s(1:Qdof, 1:nbDim, 1:nbDim+2) )
  ...
  !> for Euler fluxes
  subroutine Set_A_s_Turb2e(ndimL, nbDim, Qdof, w, A_s, xi)
   ...
   real, dimension(1:Qdof,1 :nbDim), intent(in) :: xi
    ...
    if(nbDim == 2) then
       call Set_A_s_Euler(nbDim+2, nbDim, Qdof, w(1:Qdof,1:nbDim+2), &
            A_s(1:Qdof, 1:nbDim, 1:nbDim+2, 1:nbDim+2), xi )
    elseif(nbDim == 3) then
  ...
  !> in integ nodes
  subroutine Set_K_sk_Turb2e(ndimL, Qdof, w, Dw, Re_1, K_sk)
    ...
    ! laminar terms
    if(nbDim == 2) then
       call Set_K_sk_NS(nbDim+2, nbDim, Qdof, w(1:Qdof,1:nbDim+2), Dw(1:Qdof,1:nbDim+2,1:nbDim), Re_1, &
            K_sk(1:Qdof, 1:nbDim, 1:nbDim, 1:nbDim+2, 1:nbDim+2) )
    elseif(nbDim == 3) then
       call Set_K_sk_NS(nbDim+2, nbDim, Qdof, w(1:Qdof,1:nbDim+2), Dw(1:Qdof,1:nbDim+2,1:nbDim), Re_1, &
            K_sk(1:Qdof, 1:nbDim, 1:nbDim, 1:nbDim+2, 1:nbDim+2) )
    endif
  ...
  !> in integ nodes
  subroutine Set_R_s_Turb2e(ndimL, nbDim, Qdof, w, Dw, Re_1, R_s)
    integer, intent(in) :: ndimL, nbDim, Qdof
    real, dimension(1:Qdof, 1:ndimL), intent(in):: w !state  w in #Qdof nodes
    ...
    ! laminar terms
    if(nbDim == 2) then
       call Set_R_s_NS(nbDim+2, nbDim, Qdof, w(1:Qdof,1:nbDim+2), Dw(1:Qdof,1:nbDim+2, 1:nbDim), &
