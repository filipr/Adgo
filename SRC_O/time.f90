...
contains
  ...
  !> EABDF method
  subroutine SetAdaptTimeStep(time_deg, taus, estL, tau_new, refuseStep, non_conv)
    integer, intent(in) :: time_deg
    integer, intent(in) :: non_conv
    real, dimension(1:time_deg), intent(in) :: taus
...
!         print*, 'resS, resT ' , resS , resT
       ...
       !write(*,'(a6,8es12.4)') '#@!WSE',state%estim(min_resT_S, 1) ,state%L_estim(resT) / state%L_estim(resS), coef1, coef
    ...
    if(taus(1) * coef <  min_cfl/state%max_eigenvals) then
  ...
  subroutine PrepareNewTimeStepAD(iter)
    ...
    elseif(state%time%tau_choice == 'adapt') then
       ...
       if( (state%type_IC == 0 .or. state%type_IC == -2) .and.  &
       ...
       elseif( .not. state%time%keep_tau .and. state%time%iter_loc <= 1  ) then
          ...
          ! state%time%tau_new was given in SetAdaptTimeStep
          state%time%tau(1) = state%time%tau_new
    ...
    if( state%time%iter_loc <= 1 .and. ( state%ST_Vc >0 .or.  state%ST_Ec> 0.) ) &
         state%time%tau(1) = 1.0/state%max_eigenvals
  ...
  subroutine ProposeNewTimeStep(refuseStep )
    ...
    !estL2 = estL8
    call SetAdaptTimeStep(Tdeg_actual, state%time%tau(1:state%time%deg_actual), &
         estL2, state%time%tau_new, refuseStep, NOT_CONV)


    if(refuseStep) then
       write(*,'(a15,a5,e12.4,a5,e12.4,a2,3es12.4)') &
