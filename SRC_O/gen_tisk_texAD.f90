      ...
      !print*,'#######',type, type_tab, vertical_tab
...
!      write(itex,*) '\input {description.tex}'
      ...
      if (time_method .eq. 'STDG') then
         write(itex1,'(a8,i6,a5,i1,a17,i2,a20)') &
              ...
              iBDF,'- gen_tisk_texAD - type = scalarST'
...
!               Z_plot='   {mesh-'
         ...
         elseif(type .eq. 'scalar11' .or. type .eq. 'scalar66' ) then
            if(ifig .eq. 0) then
               ncol = 2
               width = 9.
!               Z_plot='   {mesh-'
