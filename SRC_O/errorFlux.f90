...
contains
  ...
  !> the space-time dual norm by FISHPACK
  subroutine SetRHSDualSTerror( )
    ...
    Nx = 4  ! number of subelements for each bi-triangle
    ...
    do k=1, Nt   ! time levels
       ...
       do i=1, N
          do i1 = 1, Nx                   ! subsquares of element
             ...
             do j = 1, N
                ie = 2*((i-1)*N + j) - 1
                elem1 => grid%elem(ie)          ! elem1 and elem2 forms a square
  ...
  subroutine EvalDualSubElem(N, Nx, Nt, k, i, j, i1, j1, xmin, xmax, elem1, elem2, &
    ...
    real, dimension (:), allocatable :: f, Re_1
    ...
    do ie = 1,nedges   ! all edges
       ...
       rlen = 1. / Nx
       if(ie > 4) rlen = rlen / 2   ! diagonal sugfaces have a half size
       ...
       if(inside)  then  !! sub-face with a jump of the flux ??
          ...
          if(elem%face(neigh, iface) > 0) then ! interior edge
             ...
             do gk = 1, Gdof
                ...
                ! diffusive terms
                call Set_R_s(ndim, nbDim, Qdof, wi(1:Qdof,1:ndim), Dwx(1:Qdof, 1:ndim, 1:nbDim), Re_1,&
                ...
                ! diffusive terms
                call Set_R_s(ndim, nbDim, Qdof, wiR(1:Qdof,1:ndim), DwxR(1:Qdof, 1:ndim, 1:nbDim),Re_1,&
                     R_sR(1:Qdof, 1:nbDim, 1:ndim) )
...
100 continue
  ...
  !> nodes are in a square formes by two triangles
  ...
  subroutine ComputeFluxErrorElem(Set_R_s, Set_f_s, elem )
    ...
    real, dimension(:), allocatable :: val, Re_1
    ...
    do j=1, Gdof
       ...
       ! DIFFUSIVE TERMS
       ! exact value
       call Set_R_s(ndim, nbDim, Qdof, wE(1:Qdof,1:ndim), DwE(1:Qdof, 1:ndim, 1:nbDim), Re_1,&
            R_sE(1:Qdof, 1:nbDim, 1:ndim) )

       ! appoximate value
       call Set_R_s(ndim, nbDim, Qdof, wi(3, 1:Qdof,1:ndim), Dwi(3, 1:Qdof, 1:ndim, 1:nbDim), Re_1,&
            R_s(1:Qdof, 1:nbDim, 1:ndim) )
