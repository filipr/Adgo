program SetOrder
  ...
  if (command_argument_count() == 3) then
    ...
    print*,'% type of table: IPG RES STDGM  AMAsteady AMAtdp AMAst  AMAstO RES_STDGM', &
      ' RES_STDGMp pNeu pNeuP pNeuAM pNeuHP HO_rec RTNst'
    stop
 endif
 ...
 !!typ = 'CFD-AMA'
  ...
  Nz = 8
  i_tol = 8 ! state%space%adapt%tol_max
  i_CW  = 9
  i_LAiter = 10
  i_IPG = 11
...
!      ( cTn**(-1.0) / x(i,2)**2.0 ) * y(i,i_errL2H1)**2.0 )
  ...
  elseif(indexi == 5) then ! 1/(ST DOF)
     xlog(1:imesh,1) = (x(1:imesh, 3) / idof(1:imesh) ) **(1./3)
  ...
  elseif(typ == 'RES' ) then
     ...
     do i=imesh0,imesh
        ...
        write(ife, 701) y(i,i_resS) / y(i,i_XX)    ! i_eff
        write(ife, 301) z(i,i_CPU)  ! CPU time
        !write(ife, 300)
     ...
     text(1) = "$\|u - \Pi_{\hp} u\|_{L^2}$"
     text(2) = "$\|u - \Pi_{\hp} u\|_{L^2}/\omega$"
     !text(2) = "$\| E_I\|_{L^2}$"
     !text(3) = "$\|u - \Pi_h u\|_{L^\infty}$"
     !text(4) = "$\| E_I\|_{L^\infty}$"
     ...
     write(ife_b, *) "\begin{tabular}{crr|cc|c}" !c|c}"
     write(ife_b, *) "\hline"
     write(ife_b, *) " $\omega $ & $\#\Thp $ &  $\Nhp$ "
     ...
     do i=imesh0,imesh
        ...
        write(ife, '(es9.1)')  z(i,i_tol)
        !write(ife, 99) x(i,2), x(i,3)  ! h_aver, tau
        !write(ife, 100) ipar(i, 1:2)   ! P_k, T_m, ..
        !write(ife, 95) ipar(i,3)   !  Q_k
        write(ife, 95) ix(i)   !  nelem
        write(ife, 95) idof(i)   !  DOF,
  ...
  else if(typ == 'pNeu' .or. typ == 'pNeuP' .or. typ == 'pNeuAM' .or. typ == 'pNeuHP'  ) then
     ...
     elseif(typ == 'pNeuHP') then
        write(ife_b, *) "\begin{tabular}{|rr|c|ccccc|r|c|}"
     ...
     do i=imesh0,imesh
        ...
        if(typ == 'pNeuAMa') then
           ...
           if(i>1)  write(ife, '(a25,i3,a4 )') '$\approx\frac{h_0}{',ii,'}$ '
           !write(ife, 98) x(i,2)
           write(ife, 90) ipar(i, 1:1)
        else
           if(typ == 'pNeuHP')write(ife, '(i4, a4)') i-1,' & '
           write(ife, 78) ix(i)   ! nelem
           if(typ /= 'pNeuHP') write(ife, 90) ipar(i, 1:1)           ! P_k, ..
           if(typ /= 'pNeuAM') write(ife, 95) idof(i)           ! dof
        ...
        write(ife, 701) y(i,i_P_tot) / y(i,i_errH1_discrete)    ! i_eff
     ...
     text(1) = "$\|e_h\|_{L^2}$"
     text(2) = "$\eta_{L^2}(u_h)$"
     text(3) = "$\theta_{L^2}(u_h)$"
     text(4) = "$i_{L^2}$"
     text(5) = "$\theta(u_h)/\eta(u_h)$"
     !text(5) = "$i_{L^2}^{p+2}$"
     !text(6) = "$\theta_{L^2}^p$"
     !text(6) = "$\theta_{L^2}^{p+1}$"
     !text(7) = "$\theta_{L^2}^{p+2}$"

     text(6) = "$\|e_h\|_{H^1}$"
     text(7) = "$\eta_{H^1}(u_h)$"
     text(8) = "$\theta_{H^1}( u_h)$"
     text(9) = "$i_{H^1}$"
     text(10) = "$\theta(u_h)/\eta(u_h)$"

     !text(8) = "$\|e_h\|_{H^1}$"
     !!text(8) = "$\eta_{H^1}^p$"
     !text(9) = "$\eta_{H^1}^{p+1}$"
     !text(10) = "$\eta_{H^1}^{p+2}$"
     !text(11) = "$i_{H^1}^{p+1}$"
     !text(12) = "$i_{H^1}^{p+2}$"
     !!text(14) = "$\theta_{H^1}^p$"
     !text(13) = "$\theta_{H^1}^{p+1}$"
     !text(14) = "$\theta_{H^1}^{p+2}$"
     ...
     if(.not. ordA) then
        write(ife_b, *) "\begin{tabular}{cc||ccc|c|c||ccc|c|c|}"
        write(ife_b, *) "\hline"
        if( typ == 'HO_rec') write(ife_b, *) " $h$  &  $p$  "
        if( typ == 'HO_recA') write(ife_b, *) " $l$  &  DOF  "
     else
     ...
     do i=imesh0,imesh
        ...
        if(.not. ordA) then
           ...
           write(ife, 521)     y(i,i_HO_estim_L2_p2) / y(i,i_errL2)
           write(ife, 281)  y(i,i_HO_trunc_L2_p2)/y(i,i_HO_estim_L2_p2)
   ...
   !  text(1) = "$\|e_h\|_{L^2(L^2)}$"
     ...
     text(4) = "$ \eta_{rez} $"
     text(5) = "$ \eta_{flux} $"
     text(6) = "$ \|e_h\|_{old} $" ! text(6) = "$ \eta_{rad} $"
     text(7) = "$ \eta_{rad2} $"
     text(8) = "$ J(u) $"
     ...
     do i=imesh0,imesh
        ...
        if(i > 1) then
           ...
           write(ife, 952) y(i,i_RTNstNormOld), ord(i,i_RTNstNormOld) ! write(ife, 952) y(i,i_RTN_radau), ord(i,i_RTN_radau)
           ...
           write(ife, 992) y(i,i_RTNstNormOld) ! write(ife, 992) y(i,i_RTN_radau)
